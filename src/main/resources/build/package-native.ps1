...
# Define paths
# Script is now in hs-script-app/src/main/resources/build/, need to go up 5 levels to project root
$ProjectRoot = Resolve-Path (Join-Path $PSScriptRoot "..\..\..\..") | Select-Object -ExpandProperty Path
$NativeDir = Join-Path $ProjectRoot "target\native"
$ResourcesDir = Join-Path $ProjectRoot "src\main\resources"
$HsCardsDb = Join-Path $ProjectRoot "hs_cards.db"
$LogBackFile = Join-Path $ProjectRoot "src\main\resources\logback.xml"
...
try {
    ...
    if (Test-Path $LogBackFile) {
        Copy-Item $LogBackFile -Destination $TempDir
        Write-Host "  Added: logback.xml"
    } else {
        Write-Warning "Not found: logback.xml"
    }


    # Create zip file
    $ZipPath = Join-Path $ProjectRoot "target\$OutputName.zip"
